...
class DistilBertTextClassifierModelTest {
    ...
    void classify_positiveText_returnsPositiveWithHighConfidence() {
        ...
        assertThat(results.isEmpty()).as("Should return classifications").isFalse();
        assertThat(results.get(0).label()).isEqualTo("POSITIVE");
        assertThat(results.get(0).confidence() > 0.9f).as("Positive text should have high confidence, got: " + results.get(0).confidence()).isTrue();
    }
    ...
    void classify_negativeText_returnsNegativeWithHighConfidence() {
        ...
        assertThat(results.get(0).confidence() > 0.9f).as("Negative text should have high confidence, got: " + results.get(0).confidence()).isTrue();
    }
